...
$arrayFiltrado = array_filter($pessoas, function($pessoa){
    ...
    $cidade = $_POST['cidade'];
    $idade = $_POST['idade'];
    $idadeCalculada = calculaIdade($pessoa['dtNascimento']);
    $estado = $_POST['estado'];
    if((empty($nome) || (stripos($pessoa['nome'], $nome) > -1)) && (empty($idade) || $idadeCalculada == $idade) && (empty($cidade) || (stripos($pessoa['cidade'], $cidade) > -1)) && (empty($estado) || $estado == $pessoa['estado'])){
        return true;
    }
});
...
<head>
    ...
    <title>Gerar tabela filtrada</title>
</head>
<body>
    <?= $gerarTabela($arrayFiltrado) ?>
</body>
</html>